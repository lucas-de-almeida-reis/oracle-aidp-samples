"""
Multi-MCP Chat Agent (hardened) — session-variable auth, local + AIDP.

Sibling of ../agent.py with the credential/token MINTING removed and these
hardening features added:

  • Runs BOTH on AIDP and LOCALLY. When aidputils is importable (AIDP) it uses
    the OCI principal signer; otherwise it uses local OCI API-key auth via
    langchain-oci's ChatOCIGenAI (set llm.auth: api_key). Same AgentBasic,
    same create_react_agent — just a different signer.
  • Serialized MCP tool calls (_serialize_mcp_tools): OAC's ephemeral session
    deadlocks on parallel requests, so tool calls run one-at-a-time with a
    timeout + retry (matters for models that BATCH tool_calls).
  • Config-driven prompt: tune behavior in config.yaml (prompt.system_prompt /
    prompt.extra_rules) — no code edits. Domain rules live in config, reusable.

Kept from ../agent.py: MultiServerMCPClient + create_react_agent, live tool
discovery, and the full context-management stack (turn cap, token-budget trim,
orphan-tool_call healing, /reset, context-overflow + INVALID_CHAT_HISTORY
recovery).
...
# aidputils only exists inside the AIDP runtime. Import it when present (AIDP);
# otherwise fall back to local stand-ins so this SAME file also runs LOCALLY
# (outside AIDP) with OCI API-key auth (set llm.auth: api_key in config.yaml).
try:
    ...
    _AIDPUTILS_AVAILABLE = True
except ImportError:
    _AIDPUTILS_AVAILABLE = False
    OCIAIConf = None

    def init_oci_llm(conf):
        raise RuntimeError("aidputils unavailable — set llm.auth: api_key to run locally")

    def pre_invoke_setup(**kwargs):
        # AIDP injects the OCI signer + thread config here; locally we only need
        # the LangGraph thread config (bearers come from kwargs session_variables).
        tid = kwargs.get("thread_id") or kwargs.get("threadId") or "local-session"
        return {"configurable": {"thread_id": str(tid)}}

    def pre_tool_setup(**kwargs):
        # AIDP loads session variables into a contextvar here; locally we read
        # them straight from kwargs, so this is a no-op that returns no token.
        return None

    def post_tool_setup(token, **kwargs):
        return None
...
# Auth mode: "principal" = AIDP signer (init_oci_llm); "api_key" = local OCI
# config + .pem via ChatOCIGenAI. Defaults to principal on AIDP, api_key locally.
LLM_AUTH            = (CFG.get("llm", {}).get("auth", "").strip().lower()
                      or ("principal" if _AIDPUTILS_AVAILABLE else "api_key"))
LLM_OCI_CONFIG_FILE = CFG.get("llm", {}).get("oci_config_file", "~/.oci/config")
LLM_OCI_PROFILE     = CFG.get("llm", {}).get("oci_config_profile", "DEFAULT")
...
_INTEGRATION_CFG = {"adb": ADB_CFG, "oac": OAC_CFG, "oic": OIC_CFG}

# Prompt customization from config.yaml (tune the agent WITHOUT editing code):
#   prompt.system_prompt : full verbatim override (used as-is when set)
#   prompt.extra_rules   : list of extra rule lines appended after the built-ins
...
def _build_system_prompt() -> str:
    ...
    Precedence:
      1. config.yaml  prompt.system_prompt  → used VERBATIM (full override).
      2. otherwise: the built-in generic prompt (the SYSTEM_PROMPT_* / RULE_*
         constants at the top of this file — edit there), then any
         config.yaml  prompt.extra_rules  appended. Put per-deployment /
         domain rules in config so they're reusable without editing code.
    """
    override = PROMPT_CFG.get("system_prompt")
    ...
    # Per-deployment / domain rules from config.yaml (edit there, no code change).
    for rule in (PROMPT_CFG.get("extra_rules") or []):
...
# AIDP (principal) LLM config — only built when aidputils is present.
llm_conf = OCIAIConf(
    ...
    guardrails_config={},
) if _AIDPUTILS_AVAILABLE else None


def _build_local_llm():
    """LOCAL LLM: OCI Generative AI via API-key auth — no AIDP signer. Uses
    langchain-oci's ChatOCIGenAI with the same GENERIC provider the AIDP path
    uses, so create_react_agent / bind_tools behave identically."""
    from pathlib import Path as _Path
    from langchain_oci.chat_models import ChatOCIGenAI
    return ChatOCIGenAI(
        model_id=LLM_MODEL_ID,
        service_endpoint=f"https://inference.generativeai.{LLM_REGION}.oci.oraclecloud.com",
        compartment_id=LLM_COMPARTMENT_ID,
        provider="generic",
        auth_type="API_KEY",
        auth_profile=LLM_OCI_PROFILE,
        auth_file_location=str(_Path(LLM_OCI_CONFIG_FILE).expanduser()),
        model_kwargs=LLM_MODEL_ARGS or {},
    )
...
class AgentBasic:
    def __init__(self) -> None:
        ...
        # Serializes MCP tool calls (OAC's ephemeral session deadlocks on
        # parallel requests). Created lazily on first MCP load.
        self._mcp_call_lock = None
    ...
    def setup(self) -> None:
        ...
        if LLM_AUTH == "api_key":
            self.llm = _build_local_llm()
            logger.info("LLM via local OCI API-key auth (ChatOCIGenAI); model=%s", LLM_MODEL_ID)
        else:
    ...
    def _resolve_bearers(self, kwargs: dict) -> dict:
        ...
        for name in ("adb", "oac", "oic"):
            ...
            # Local/dev fallback: when no session variable is present (e.g. running
            # outside AIDP), use integrations.<name>.dev_bearer from config.yaml.
            if not b:
                dev = (_INTEGRATION_CFG.get(name) or {}).get("dev_bearer")
    ...
    async def _load_all_mcp_tools(self, bearers: dict) -> None:
        ...
        if self._mcp_call_lock is None:
            self._mcp_call_lock = asyncio.Lock()
        tools = self._serialize_mcp_tools(await client.get_tools())
    ...
    def _serialize_mcp_tools(self, tools):
        """Wrap each MCP tool's async entrypoint in a shared lock so concurrent
        tool calls run ONE-AT-A-TIME. OAC's ephemeral streamable-HTTP session
        deadlocks on parallel requests, and LangGraph's ToolNode runs a step's
        tool_calls concurrently — so a model that BATCHES tool_calls (e.g.
        gpt-5.5 firing several describe_data at once) makes them all fail
        without this (grok-4.3 calls sequentially, so it never hit it). Also
        bounds each call with a 30s timeout + 1 retry, returning an error
        STRING (not raising) so one slow call can't hang the run, and short-
        circuits describe_data called with empty/missing args."""
        lock = self._mcp_call_lock
        for t in tools:
            orig = getattr(t, "coroutine", None)
            if orig is None:
                continue

            async def _locked(*args, __orig=orig, __name=getattr(t, "name", "tool"), **kwargs):
                _has_dm = kwargs.get("datamodelName") or any(
                    isinstance(a, dict) and a.get("datamodelName") for a in args)
                if "describe_data" in (__name or "") and not _has_dm:
                    return ("ERROR: describe_data needs BOTH 'datamodelName' and 'tableName'. "
                            "Call describe_data once per table with both filled in — "
                            "never with empty arguments.")
                async with lock:
                    for attempt in range(2):
                        try:
                            return await asyncio.wait_for(__orig(*args, **kwargs), timeout=30)
                        except asyncio.TimeoutError:
                            logger.warning("MCP tool %s timed out (attempt %d/2)", __name, attempt + 1)
                    return (f"ERROR: the {__name} tool did not respond in time — the OAC MCP "
                            f"connection appears slow/unstable right now. Try the same call again.")

            try:
                t.coroutine = _locked
            except Exception:
                try:
                    object.__setattr__(t, "coroutine", _locked)
                except Exception:
                    logger.warning("Could not serialize MCP tool %s", getattr(t, "name", "?"))
        return tools
...
# ── Local run (outside AIDP) ─────────────────────────────────────────────────
# Usage:  python agent.py "your question"
# Needs config.yaml with llm.auth: api_key + a valid local OCI config/.pem, and a
# bearer for each enabled integration via integrations.<x>.dev_bearer (or pass
# session_variables in kwargs). Runs the SAME AgentBasic used on AIDP.
if __name__ == "__main__":
    import asyncio as _asyncio, sys as _sys, uuid as _uuid

    async def _run_local():
        question = " ".join(_sys.argv[1:]).strip() or "What datasets are available?"
        agent = AgentBasic()
        agent.setup()
        if getattr(agent, "_self_stage_error", None):
            print("SETUP ERROR:\n", agent._self_stage_error)
            return
        result = await agent.invoke(question, thread_id=_uuid.uuid4().hex)
        msgs = result.get("messages", []) if isinstance(result, dict) else []
        print("\n=== Q:", question)
        print("=== ANSWER ===")
        if msgs:
            m = msgs[-1]
            print(getattr(m, "content", None) if not isinstance(m, dict) else m.get("content"))
        else:
            print(result)

    _asyncio.run(_run_local())
